<template>
    <div class="row">
        <div class="col-12">
            <div class="card">
                <form id="UkuranForm" action="" :method="id ? 'put' : 'post'" enctype="multipart/form-data">
                    <div class="card-header">
                        <div class="row">
                            <div class="col">
                                <h1>{{ id ? "Edit" : "Add" }} Ukuran</h1>
                            </div>
...
<script>
    ...
    export default {
        ...
        mounted() {
            let form = [this.id ? "Edit" : "Add", "Ukuran"].join(" ")
            if (this.id)
            {
        ...
        methods: {
            getDataUkuran(id) {
                axios.get('/api/ukuran/' + id).then((response) => {
            ...
            handleSubmit() {
                ...
                if (this.id)
                {
                    axios.put('/api/ukuran/edit/' + this.id, this.$data.formData)
                    .then((response) => {
                        console.log(response)
                ...
                {
                    axios.post('/api/ukuran/add', this.$data.formData)
                    .then((response) => {
                        console.log(response)
                        if (response.data.status)
                        {
            ...
            handleDelete() {
                ...
                if (flag)
                {
                    axios.delete('/api/ukuran/delete/' + this.id).then((response) => {
                        console.log(response)
                        if (response.data.status)
                        {
